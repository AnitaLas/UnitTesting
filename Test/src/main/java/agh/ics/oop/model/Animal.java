    ...
    public void move(MoveDirection direction){
        switch (direction){
            ...
            case FORWARD -> {
                ...
                if(position.add(to_add).getX() <= 4
                        && position.add(to_add).getY() <= 4
                        && position.add(to_add).getX() >= 0
                        && position.add(to_add).getY() >= 0){
                    position = position.add(to_add);
                }
            }
            case BACKWARD -> {
                Vector2d to_sub = MapDirection.toUnitVector(orientation);
                if(position.subtract(to_sub).getX() <= 4
                        && position.subtract(to_sub).getY() <= 4
                        && position.subtract(to_sub).getX() >= 0
                        && position.subtract(to_sub).getY() >= 0){
